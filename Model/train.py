...
from data import NerDataloader
import os
...
# dataset parameter
parser.add_argument('--train_data', type=str, default='dataset/ner/NCBI-disease/train.tsv')
parser.add_argument('--valid_data', type=str, default='dataset/ner/NCBI-disease/devel.tsv')
...
# data_loader
train_data_loader =  NerDataloader(cfg.config['train_data'], cfg.config['batch_size'], tokenizer, logger, cfg.config['shuffle'], cfg.config['max_seq_length'])
valid_data_loader = NerDataloader(cfg.config['valid_data'], cfg.config['batch_size'], tokenizer, logger, cfg.config['shuffle'], cfg.config['max_seq_length'])
...
model = NER(n_tag=len(train_data_loader.label2id), tag2id=train_data_loader.label2id, vocab_size=len(vocab), device=device, checkpoint=checkpoint,**cfg.config)
model.to(device)
logger.info(model)
...
# criterion = nn.CrossEntropyLoss(ignore_index=tokenizer._token_pad_id)

#trainer
trainer = NerTrainer(model=model, optimizer=optimizer, criterion=None, cfg=cfg.config, logger=logger, 
